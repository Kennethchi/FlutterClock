...
import 'animated_effect_background.dart';
...
class _RainyEffectState extends State<RainyEffect> with TickerProviderStateMixin<RainyEffect> {
  @override
  Widget build(BuildContext context) {
    return AnimatedEffectBackground(
      behaviour: MovingLinesBehaviour(
...
class MovingLinesBehaviour extends Behaviour {
  static final math.Random random = math.Random();

  /// Creates a new racing lines behaviour
  MovingLinesBehaviour({this.direction = LineDirection.Ltr, int numLines = 50})
  ...
  void initLine(Line line) {
    ...
    line.speed = random.nextDouble() * 2000 + 500;

    final bool axisHorizontal = (direction == LineDirection.Ltr || direction == LineDirection.Rtl);
    final bool normalDirection = (direction == LineDirection.Ltr || direction == LineDirection.Ttb);
    ...
    line.color = Colors.blue.shade50.withOpacity(0.5);
  }
  ...
  void initFrom(Behaviour oldBehaviour) {
    if (oldBehaviour is MovingLinesBehaviour) {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final int sign = (direction == LineDirection.Ltr || direction == LineDirection.Ttb) ? 1 : -1;
    ...
    for (var line in lines) {
      ...
      paint.shader = ui.Gradient.linear(line.position, target - headDelta, <Color>[line.color.withAlpha(0), line.color]);
      paint.strokeWidth = line.thickness.toDouble();
